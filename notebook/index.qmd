...
- **Read this INDEX first.** If you only want the answer, go straight to the
  **[results page](results.qmd)** — the headline two numbers, the measured catalog, the completeness
  bound, the campaign figures, and the systematics ledger in one place. Otherwise open whichever of
  the four thread files you need. Each page opens with an **"In plain terms"** box summarizing what
  it covers and how it ended, so you can re-orient in 30 seconds after a long time away.
- **Hit an unfamiliar word, symbol, or piece of shorthand?** Everything is defined in plain words in
  the **[glossary](glossary.qmd)** — including project coinages like *the pedestal*, *V_p*,
  *caldet*, *WS1/WS2/WS3*, *POP1/POP2*, and which of several models is the current one.
- **Cross-references** between pages are ordinary links (e.g. [physics](physics.qmd)); use the
  sidebar to jump between chapters.
- **When a page and an archived original disagree, the page wins** — it folds in later corrections.
- **Figures** are generated live at render time from the real analysis caches by
  [figures/make_figures.py](figures/make_figures.py) (the pedestal figure in
  [dipole_algorithm](dipole_algorithm.qmd) is the first example) — see that folder's README to add or
  refresh one. The static-Markdown copy of the notebook keeps them as committed SVGs under
  [figures/](figures/).
...
1. **[The dipole algorithm](dipole_algorithm.qmd)** — how traps are found, fit, and graded.
...
2. **[The signed-refit investigation](signed_refit.qmd)** — the multi-month exploration
...
3. **[Completeness & efficiency](completeness_efficiency.qmd)** — is the catalog missing a
...
4. **[Simulation physics](physics.qmd)** — the SRH capture/emission/recapture model in the
...
What the four threads *produce* is collected on a fifth page, **[Results](results.qmd)** — the
two-number headline, the measured catalog (E, σ, τ135), the completeness bound and upper-limit
population, the simulation campaign figures, and the systematics ledger. Threads 1–4 are the
*how*; the results page is the *what*.
...
|---|---|---|
| [dipole_algorithm](dipole_algorithm.qmd) | catalog | code map (entry point → cached stages), finder / fit / grading, legacy vs minimal, calibrated detection table |
| [signed_refit](signed_refit.qmd) | catalog method | decision-first: the kept/dropped table, then the trigger, the surviving corrections, and the rejected machinery + adversarial reviews |
| [completeness_efficiency](completeness_efficiency.qmd) | completeness | Methods 1/2/3, the stage-by-stage pipeline (`run_chain.py`), Method-3 results, the naive-dip closure, high-T fading split, upper-limit seeding |
| [physics](physics.qmd) | simulation | code map (entry points → the `CCD` object), emission physics, three recapture models, phase-limited deviation budget, operating conditions, deferred HEE sampling |
| [results](results.qmd) | **the answer** | the two-number headline (null + 90% CL allowance), the measured catalog, completeness bound + upper-limit population, campaign figures, systematics ledger |
| [glossary](glossary.qmd) | — | plain-language definitions of every term, symbol, and piece of project shorthand used above |

Each notebook page is self-contained (derivations, equations, and numeric tables folded in), opens
with an "In plain terms" summary, and cross-links the others. Any term you don't recognize is in the
...
| `DEVIATION_BUDGET.md` | physics (+ completeness §8) | current content preserved; it *is* the current physics state |
...
| `SIGNED_REFIT_AUDIT.md` | signed_refit §3 | first change audit (pedestal discovery / trigger) |
| `SIGNED_REFIT_PHYSICS_AUDIT.md` | signed_refit §4–§5 | physics audit (4 problems + refit sequence) |
| `SIGNED_REFIT_RUNBOOK.md` | signed_refit §4–§5 | the full Step 1–10 machinery (not adopted as production) |
| `SIGNED_REFIT_RUNBOOK_S01_S05_AUDIT.md` | signed_refit §5 | **redundant** — earlier draft of the Steps 1–5 review |
| `SIGNED_REFIT_STEP1_5_REVIEW.md` | signed_refit §5 | adversarial review Steps 1–5 |
| `SIGNED_REFIT_STEP6_10_REVIEW.md` | signed_refit §5 | adversarial review Steps 6–10 |
| `MINIMAL_SIGNED_REFIT_RECOMMENDATION.md` | signed_refit §1 | **the decision** |
| `signed_refit_manifest_summary.md` | signed_refit §5 (Step 1) | per-step report (acquisition-family confound) |
| `signed_refit_control_pair_summary.md` | signed_refit §5 (Step 2) | per-step report |
| `signed_refit_noise_model_report.md` | signed_refit §5 (Steps 3–4) | per-step report |
| `signed_refit_noise_closure.md` | signed_refit §5 (Step 4) | per-step report (non-closure numbers) |
| `signed_refit_profile_fitter_validation.md` | signed_refit §4 (Step 5, kept) | per-step report |
| `signed_refit_detection_calibration.md` | signed_refit §4 / dipole_algorithm §3.1 | per-step report (threshold table) |
| `signed_refit_finder_calibration.md` | signed_refit §4 (Step 7, kept) | per-step report (finder tradeoff) |
| `signed_refit_orientation_policy.md` | signed_refit §4 (Step 8, kept) | per-step report |
| `signed_refit_intensity_cutflow.md` | signed_refit §5 (Step 9) | per-step report (cutflow) |
| `signed_refit_candidate_variance_closure.md` | signed_refit §5 (R1 v1) | per-step report (φ knob, superseded by v2) |
| `signed_refit_candidate_variance_closure_v2.md` | signed_refit §5 (R1) | per-step report (φ = 8–30) |
| `signed_refit_variance_validation.md` | signed_refit §5 | per-step report |
| `signed_refit_srh_validation.md` | signed_refit §5 (Step 10) | per-step report (SRH catalog numbers) |
| `trap_completeness_method.md` | completeness_efficiency §§1–2 | conceptual design (Methods 1/2/3) |
| `NAIVE_EFFICIENCY_DIP_EXPLANATION.md` | completeness_efficiency §6 | closed result (dip = GOF artifact) |
| `HIGH_T_FAILURE_SPLIT_PLAN.md` | completeness_efficiency §7 | executed plan (fading split) |
| `MEASUREMENT_COMPLETENESS_REVIEW.html` | dipole_algorithm + completeness_efficiency | rendered 4-tab synthesis (2026-06-17/18); tab 1 → dipole_algorithm, tabs 2+4 → completeness_efficiency (Method-3 + validation/WS1-3), tab 3 (figure reference) → completeness_efficiency §4 |
...
> `dipole_algorithm.qmd` §4 and `completeness_efficiency.qmd` §4 rather than reproduced image-by-image.
...
- `trap_completeness_method3/` — **live analysis workspace** (code in `src/`, stage packets in
  ...
  [completeness_efficiency.md](completeness_efficiency.qmd), but the subtree itself is kept
  intact because it is runnable infrastructure, not a note. `agents/11_naive_efficiency_closure.md`
  and `agents/12_high_temp_misfit.md` hold the stage-level detail behind §6/§7 of that notebook file.
